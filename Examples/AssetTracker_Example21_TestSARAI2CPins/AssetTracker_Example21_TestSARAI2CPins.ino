/*
  ...
  Use jumper wires to connect: SARA SCL to D1; and SARA SDA to D0.
...
void setup()
{
  ...
  // In this example, we want to configure D0 and D1 as inputs
  pinMode(D0, INPUT);
  pinMode(D1, INPUT);
  ...
  // Define the I2C (DDC) SDA and SCL pins as test pins
  // The pin numbers are 26 and 27 (big endian)
  const char test_pins[] = "+UTEST=10,2,\"000000000000000006000000\""; // Define pins 26 and 27 as test pins
  result = assetTracker.sendCustomCommandWithResponse(test_pins, SARA_R5_RESPONSE_OK, response_ptr);
  if (result != SARA_R5_SUCCESS)
  {
    SERIAL_PORT.println(F("Could not configure the DDC pins for testing! Freezing..."));
    while (1)
      ; // Do nothing more
  }

  // Define the I2C (DDC) SDA and SCL pins as outputs
  const char set_io[] = "+UTEST=10,3,\"000000000000000000000000\""; // Define pins 26 and 27 as outputs
  result = assetTracker.sendCustomCommandWithResponse(set_io, SARA_R5_RESPONSE_OK, response_ptr);
  if (result != SARA_R5_SUCCESS)
  {
    SERIAL_PORT.println(F("Could not configure the DDC pins for I/O! Freezing..."));
    while (1)
      ; // Do nothing more
  }

  // Set the I2C (DDC) SDA and SCL pins HIGH
  const char set_high[] = "+UTEST=10,4,\"000000000000000006000000\""; // Set selected pins high
  result = assetTracker.sendCustomCommandWithResponse(set_high, SARA_R5_RESPONSE_OK, response_ptr);
  if (result != SARA_R5_SUCCESS)
  {
    SERIAL_PORT.println(F("Could not set the DDC pins high! Freezing..."));
    while (1)
      ; // Do nothing more
  }
  ...
  if (digitalRead(D0) != HIGH) SERIAL_PORT.println(F("ERROR! SARA SDA pin is not high!"));
  if (digitalRead(D1) != HIGH) SERIAL_PORT.println(F("ERROR! SARA SCL pin is not high!"));

  // Set the I2C (DDC) SDA and SCL pins LOW
  ...
  if (result != SARA_R5_SUCCESS)
  {
    SERIAL_PORT.println(F("Could not set the DDC pins low! Freezing..."));
    while (1)
      ; // Do nothing more
  }
  ...
  if (digitalRead(D0) != LOW) SERIAL_PORT.println(F("ERROR! SARA SDA pin is not low!"));
  if (digitalRead(D1) != LOW) SERIAL_PORT.println(F("ERROR! SARA SCL pin is not low!"));
